...
// La fonction qui nous permet de d'échapper les codes script dans le formulaire :
...
function displayIdeas() {
    ...
    ideas.forEach((idea, index) => {
        ...
        <div class="btn-section">
            <button class="approve-idea" data-index="${index}" ${idea.approved ? "style='display:none'" : ""}>Approuver</button>
            <button class="disapprove-idea" data-index="${index}" ${!idea.approved ? "style='display:none'" : ""}>Désapprouver</button>
            <button class="delete-idea" data-index="${index}">Supprimer</button>
        </div>
        `;

        if (idea.approved) {
            ideaItem.style.border = "2px solid green";
        } else {
            ideaItem.style.border = "2px solid red";
        }
